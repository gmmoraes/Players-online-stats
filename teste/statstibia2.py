...
# commit: removed cursor from init, transformed create_table into a private method
# removed self.a and self.c      
        
#Should data_entry be outside of the class?
#I was only getting the total number of players from the top rigth of the page, but I could try to get the number from each server
# I was defining as instance variables because they could be used by other functions, is there any way to declare the cursor in initialize and use it
# in the create_table class or shoud I declare it as an instance variable?
#In your example the open would only create the database or would the cursor be declared inside the open function with the initialize blank?
